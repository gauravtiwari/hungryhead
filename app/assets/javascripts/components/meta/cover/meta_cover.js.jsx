...
var MetaCover = React.createClass({
  getInitialState: function() {
    return {
      cover: this.props.meta.cover,
      form: this.props.meta.form,
      draggable: false,
      is_owner: this.props.meta.is_owner,
      position: this.props.meta.cover.position,
      visible: false,
      loading: false
    }
  },
  ...
  render: function() {
      ...
      if(this.state.cover !== undefined){
        var image = <img className="cover-image" id="meta_cover_preview" src={this.state.cover.url} style={imageStyle}/>;
      } else {
        ...
        return (
          <div className={drag_class} id="cover-wrap">
            <form ref="coverForm" method="PUT" action={this.state.form.action} id="cover-upload" className="cover-form" onChange={this._onChange} encType="multipart/form-data">
              ...
              <input type="file" ref="cover" style={{"display" : "none"}} name="meta[cover]" id="meta_cover" />
               <input type="hidden" ref="position" name="meta[cover_position]" value={this.state.position} />
