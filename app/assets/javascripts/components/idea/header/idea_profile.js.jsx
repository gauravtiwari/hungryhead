var IdeaProfile = React.createClass({
  ...
  render: function() {
    ...
    var market_list = _.map(this.props.idea.markets, function(tag){
      return  <a key={Math.random()} className="badge badge-white m-r-10" href={tag.url}>
            {tag.name}
          </a>;
    });

    if(this.props.idea.looking_for_team) {
      var looking_for_team = <div className="badge badge-white m-t-10"><span className='fa fa-search'></span> Looking for team</div>;
    }
