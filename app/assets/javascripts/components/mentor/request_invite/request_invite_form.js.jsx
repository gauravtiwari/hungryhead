var RequestInviteForm = React.createClass({

  getInitialState: function() {
    return {
      submitted: $.cookie('submitted_invite_request') ? $.cookie('submitted_invite_request') : false,
      name: $.cookie('submitted_invite_request') ? $.cookie('submitted_invited_name') : null,
      loading: false
    }
  },
    ...
    if($(this.refs.form.getDOMNode()).valid()) {
      ...
      $.post(Routes.invite_requests_path(), formData, function(data, textStatus, xhr) {
        if(data.created) {
          ...
          $.cookie('submitted_invite_request', true, {expires: 365});
          $.cookie('submitted_invited_name', data.name, {expires: 365});
  ...
  clearCookie: function() {
    $.removeCookie('submitted_invite_request');
    $.removeCookie('submitted_invited_name');
       ...
       <div className="join-alumni-form clearfix">
        ...
        <div className="col-sm-8 col-md-8 auto-margin">
          ...
          <p className="fs-12 pull-right text-right m-t-20 text-master">
            We will be in touch within next 24 hours.
          </p>
        </div>
      </div>
      </form>;
