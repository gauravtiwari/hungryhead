class BadgesController < ApplicationController
  ...
  def show
    ...
    if @badge.custom_fields[:type] == "user"
      ...
      @ideas = @badge.custom_fields[:type].titlecase.constantize.where(sash_id: Merit::BadgesSash.where(badge_id: @badge.id).pluck(:sash_id)).paginate(:page => params[:page], :per_page => 20)
      render :ideas
    end
  end
end
