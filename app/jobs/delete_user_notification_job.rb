  ...
  def perform(trackable_id, trackable_type)
    ActiveRecord::Base.connection_pool.with_connection do
      Notification.where(trackable_id: trackable_id, trackable_type: trackable_type).find_each do |notification|
        notification.user.latest_notifications.remrangebyscore(notification.created_at.to_i + notification.id, notification.created_at.to_i + notification.id)

        #Find followers for this notification and remove notification from it's feed
        find_followers(notification).each do |f|
          f.latest_notifications.remrangebyscore(notification.created_at.to_i + notification.id, notification.created_at.to_i + notification.id)
        end

        #if recipient is idea, remove from the idea feed
        if notification.recipient_type == "Idea"
          notification.recipient.latest_notifications.remrangebyscore(notification.created_at.to_i + notification.id, notification.created_at.to_i + notification.id)
        end
